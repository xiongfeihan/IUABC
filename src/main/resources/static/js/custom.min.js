...
function init_gauge() {
    if ("undefined" != typeof Gauge) {
        ...
        if ($("#gauge-text1").length && (c.maxValue = 60, c.animationSpeed = 100, c.set(parseFloat($("#cart_speed").text()))), $("#chart_gauge_02").length)
            var d = document.getElementById("chart_gauge_02"),
            e = new Gauge(d).setOptions(a);
        $("#gauge-text2").length && (e.maxValue = 40, e.animationSpeed = 100, e.set(15))
        var f = document.getElementById("chart_gauge_03"),
            g = new Gauge(f).setOptions(a);
        $("#gauge-text3").length && (g.maxValue = 6, g.animationSpeed = 100, g.set(4))
    }
}
